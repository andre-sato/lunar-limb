...
```mermaid
sequenceDiagram
    participant Orbita as Órbita
    participant Voce as Seu endpoint

    Orbita->>Voce: POST /webhooks/orbita
    Note over Voce: processa
    Voce-->>Orbita: 200
    Note over Orbita: entregue

    Orbita->>Voce: POST /webhooks/orbita
    Voce-->>Orbita: 500
    Note over Orbita: reenvia em 1min, 5min, 30min, 2h, 12h
```
...
:::caution[Pelo menos uma vez, nunca exatamente uma vez]
...
export function verificar(corpo: string, cabecalho: string, segredo: string): boolean {
...
:::danger[Verifique sobre o corpo bruto]
...
<If flag="plano" equals="enterprise">
...
| --- | --- |
| [Conteúdo condicional](/guides/conteudo-condicional/) | Os três blocos `<If>`: `beta`, `beta not` e `plano equals` |
| [Diagramas](/guides/diagramas/) | O diagrama de sequência da entrega e do reenvio |
| Componentes da Starlight | `:::caution`, `:::danger`, `:::note` |
| [Linter](/guides/linter-e-quality-score/) | Terminologia canônica e legibilidade da página |
